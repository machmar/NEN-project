#include "HUDStuff.h"
#include "fx8.h"

static const uint8_t LevelBannerData[5][76] = {
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x18, 0x07, 0x03, 0x1e, 0x18, 0x06, 0x03, 0x1c, 0x33, 0x11, 0x1f, 0x06, 0x00, 0x00, 0x00, 0x1f,
        0x11, 0x13, 0x1e, 0x00, 0x1c, 0x36, 0x22, 0x32, 0x0e, 0x00, 0x1c, 0x22, 0x36, 0x18, 0x00, 0x40,
        0x7e, 0x1a, 0x2a, 0x66, 0x00, 0x20, 0x6c, 0x4a, 0x72, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,},
    {0x00, 0x00, 0x00, 0x3c, 0x66, 0x43, 0x43, 0x40, 0x00, 0x78, 0x1f, 0x17, 0x1c, 0x78, 0x00, 0x1f,
        0x07, 0x7c, 0x78, 0x0f, 0x00, 0x00, 0x00, 0x7f, 0x00, 0x00, 0x60, 0x7c, 0x46, 0x40, 0x18, 0x3e,
        0x2a, 0x22, 0x03, 0x00, 0x20, 0x18, 0x16, 0x12, 0x3c, 0x00, 0x00, 0x1e, 0x30, 0x1c, 0x00, 0x38,
        0x4e, 0x0a, 0x02, 0x02, 0x00, 0x00, 0x02, 0x1e, 0x02, 0x1e, 0x08, 0x1e, 0x00, 0x30, 0x0e, 0x00,
        0x2e, 0x3a, 0x32, 0x00, 0x04, 0x06, 0xb3, 0x19, 0x0f, 0x06, 0x00, 0x00,},
    {0x00, 0x00, 0x02, 0x1e, 0x30, 0x1b, 0x1e, 0x30, 0x1e, 0x00, 0x00, 0x3e, 0x08, 0x08, 0x08, 0x1e,
        0x00, 0x3e, 0x0b, 0x3e, 0x00, 0x02, 0x0e, 0x1a, 0x02, 0x00, 0x00, 0x40, 0x78, 0x16, 0x38, 0x00,
        0x3f, 0x19, 0x36, 0x20, 0x1e, 0x3e, 0x2a, 0x62, 0x00, 0x00, 0x00, 0x02, 0x02, 0x02, 0x7e, 0x02,
        0x02, 0x3c, 0x0e, 0x28, 0x3e, 0x00, 0x3c, 0x2e, 0x2a, 0x22, 0x00, 0x00, 0x4e, 0x5a, 0x32, 0x00,
        0x3e, 0x6a, 0x0a, 0x00, 0x00, 0x06, 0x03, 0x5b, 0x0e, 0x00, 0x00, 0x00},
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x7f, 0x60, 0x20, 0x20, 0x00, 0x00, 0x3f, 0x69, 0x29, 0x00, 0x02,
        0x02, 0x02, 0x3e, 0x02, 0x02, 0x02, 0x00, 0x00, 0x00, 0x00, 0x30, 0x0c, 0x03, 0x0e, 0x0c, 0x72,
        0x0e, 0x00, 0xf0, 0x9c, 0x96, 0x12, 0x12, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x38, 0x2c,
        0x22, 0x22, 0x29, 0x39, 0x00, 0x00, 0x00, 0x3c, 0x46, 0x82, 0x82, 0xc2, 0x42, 0x62, 0x3c, 0x00,
        0x00, 0xb8, 0x0e, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,},
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x60, 0x18, 0x0e, 0x0b, 0x1f, 0x20, 0x00, 0x00, 0x30, 0x0c, 0x1e,
        0x20, 0x10, 0x0e, 0x00, 0x0e, 0x78, 0x08, 0x04, 0x06, 0x00, 0x7c, 0x46, 0x22, 0x3e, 0x00, 0x70,
        0x1e, 0x7e, 0xe0, 0x3e, 0x00, 0x7e, 0x4a, 0x42, 0x02, 0x00, 0x00, 0x00, 0x00, 0x7e, 0x10, 0x10,
        0x7c, 0x06, 0x70, 0x4c, 0x4e, 0x4a, 0x4a, 0x02, 0x78, 0x1f, 0x19, 0x2d, 0x67, 0x00, 0x00, 0x3f,
        0x49, 0x49, 0x40, 0x00, 0x02, 0x01, 0x59, 0x0d, 0x06, 0x00, 0x00, 0x00,},
};

static const dogm128_bitmap_t LevelBanners[5] = {
    {76, 8, LevelBannerData[0]},
    {76, 8, LevelBannerData[1]},
    {76, 8, LevelBannerData[2]},
    {76, 8, LevelBannerData[3]},
    {76, 8, LevelBannerData[4]},
};

void HUD_DrawBanner(uint8_t number) {
    if (number > 4)
        return;

    dogm128_hline(10, 0, 75, DISP_COL_WHITE);
    dogm128_hline(10, 1, 75, DISP_COL_WHITE);
    dogm128_hline(11, 2, 73, DISP_COL_WHITE);
    dogm128_hline(11, 3, 73, DISP_COL_WHITE);
    dogm128_hline(12, 4, 71, DISP_COL_WHITE);
    dogm128_hline(12, 5, 71, DISP_COL_WHITE);
    dogm128_hline(13, 6, 69, DISP_COL_WHITE);
    dogm128_hline(13, 7, 69, DISP_COL_WHITE);
    dogm128_hline(13, 8, 69, DISP_COL_BLACK);
    dogm128_line(9, 0, 12, 7, DISP_COL_BLACK);
    dogm128_line(85, 0, 82, 7, DISP_COL_BLACK);
    dogm128_blit_or(10, 0, &LevelBanners[number]);
}

void HUD_DrawMap(uint8_t x_loc, uint8_t y_loc, map_t map, player_t player) {
    int16_t static height_scroll = 0;
    uint8_t max_scroll = 0;
    if (map.height > 32)
        max_scroll = map.height - 32;
    uint8_t const top_edge_scroll_threshold = 5; // how many pixels does the player have to be from the edge to scroll the map
    uint8_t const bottom_edge_scroll_threshold = 26; // how many pixels does the player have to be from the edge to scroll the map
    
    if (FX_I(player.posY) - height_scroll < top_edge_scroll_threshold && height_scroll > 0)
        height_scroll--;
    
    if (FX_I(player.posY) - height_scroll > bottom_edge_scroll_threshold && height_scroll < max_scroll)
        height_scroll++;
    
    dogm128_blit_or(x_loc, (int16_t)y_loc - height_scroll, map.minimap);
    
    static uint8_t show = 0;
    if (show & 0b100) // make the player grey
        dogm128_pixel((uint8_t)FX_I(player.posX) + x_loc, (uint8_t)FX_I(player.posY) + (uint8_t)(y_loc - height_scroll), DISP_COL_BLACK);
    show++;
}
