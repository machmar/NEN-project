...
That means plain bitmaps are currently an external-tool plus manual integration step.

The normal bitmap workflow currently used for this repo is:

1. Prepare a 1-bit source image at the exact runtime size you need, such as `76x8` for a banner or `32x32` for a minimap.
2. Open `https://javl.github.io/image2cpp/`.
3. Enable inverted colors.
4. Set the output format to plain bytes.
5. Set byte orientation to vertical, `1 bit per pixel`.
6. Copy the generated byte array into `assets.c`.
7. Wrap that array in a `dogm128_bitmap_t` with the exact width and height.
...
- if needed, start from copied demo arrays as placeholders, then replace them with final `image2cpp` output later
